...
- Admin dashboard tại `/admin/` để tạo tài khoản admin, cấu hình Chatwoot/OA, thêm inbox, quản lý proxy, quét QR và xem log.
...
| Cấu hình | Logs | Mobile |
| --- | --- | --- |
| <img src="assets/images/admin-preview/settings-desktop.png" alt="Màn hình cấu hình Chatwoot, Zalo OA và webhook URL" width="300"> | <img src="assets/images/admin-preview/logs-desktop.png" alt="Màn hình nhật ký vận hành" width="300"> | <img src="assets/images/admin-preview/dashboard-mobile.png" alt="Dashboard admin trên viewport mobile" width="180"> |
...
4. (Tùy chọn) Vào tab **Proxy** để khai báo proxy nếu muốn định tuyến tài khoản cá nhân qua IP riêng (xem mục [Proxy và tự động kết nối lại](#proxy-và-tự-động-kết-nối-lại)).
5. Tạo tài khoản bridge:
   - Với Zalo cá nhân: thêm account, tạo hoặc gắn Chatwoot inbox, chọn proxy (nếu có), bấm đăng nhập và quét QR.
   - Với Zalo OA: thêm OA account, bấm kết nối OA và hoàn tất OAuth.
6. Copy webhook URL trong admin dashboard:
   - Chatwoot webhook: dùng cho inbox Chatwoot tương ứng.
   - Zalo OA webhook: dùng trong Zalo developer console nếu bật OA.
7. Gửi thử một tin inbound và một tin outbound để kiểm tra mapping và chống echo.
...
khi lưu. Mỗi tài khoản cá nhân có thể gắn một proxy để toàn bộ lưu lượng (websocket và HTTP) của tài
khoản đó đi qua IP riêng — hữu ích khi muốn tách IP cho mỗi tài khoản. Khi bạn đổi proxy của một tài
khoản đang chạy, tài khoản hiển thị nhãn **"Cần áp dụng proxy"**; bấm **"Áp dụng proxy"** để bridge
kết nối lại qua proxy mới. Xóa một proxy đang được dùng sẽ yêu cầu xác nhận và tự gỡ proxy khỏi các
tài khoản liên quan.
