...
describe("ReactionHandler", () => {
  it("posts a private note threaded onto the reacted message", async () => {
    const d = deps();
    await new ReactionHandler(d.conversations as any, d.mapping as any, d.appClient as any).handle(1, reaction());
  ...
  it("labels the operator's own reaction as 'Bạn'", async () => {
    const d = deps();
    await new ReactionHandler(d.conversations as any, d.mapping as any, d.appClient as any).handle(1, reaction({ isSelf: true }));
  ...
  it("ignores a reaction removal (empty icon)", async () => {
    const d = deps();
    await new ReactionHandler(d.conversations as any, d.mapping as any, d.appClient as any).handle(1, reaction({ icon: "" }));
...
describe("UndoHandler", () => {
  it("deletes the mapped Chatwoot message when the operator recalls their own message", async () => {
    ...
    await new UndoHandler(d.conversations as any, d.mapping as any, d.appClient as any).handle(1, undo());
  ...
  it("ignores a customer recall (not isSelf)", async () => {
    ...
    await new UndoHandler(d.conversations as any, d.mapping as any, d.appClient as any).handle(1, undo({ isSelf: false }));
