...
describe("makeOutboundNotifier", () => {
  it("posts a private note to the resolved conversation", async () => {
    const d = deps();
    const notify = makeOutboundNotifier((id) => (id === 3 ? "ident-1" : null), d.accounts as any, d.conversations as any, d.appClient as any);
  ...
  it("does nothing when the inbox cannot be resolved", async () => {
    const d = deps();
    const notify = makeOutboundNotifier(() => null, d.accounts as any, d.conversations as any, d.appClient as any);
    await notify(evt as any, "boom");
  ...
  it("does nothing when there is no conversation yet", async () => {
    ...
    const notify = makeOutboundNotifier(() => "ident-1", d.accounts as any, d.conversations as any, d.appClient as any);
    await notify(evt as any, "boom");
    expect(d.appClient.postPrivateNote).not.toHaveBeenCalled();
  });
